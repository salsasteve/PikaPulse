...
use spectrum_analyzer::{windows::hann_window, samples_fft_to_spectrum, FrequencyLimit, FrequencyValue, scaling::divide_by_N, FrequencySpectrum};
use std::cmp::max;
use std::cell::Ref;
...
fn update(_app: &App, model: &mut Model, _update: Update) {
    let latest_audio_data = model.recorder.get_latest_audio_data().lock().unwrap().to_vec();
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    rainbow_circle3(&draw, &spectrum_data, 0.1, win, 1.0);
...
fn select_recent_samples(audio: &[f32], sample_count: usize) -> Vec<f32> {
    audio.iter().skip(audio.len() - sample_count).cloned().collect()
}

fn perform_fft(samples: &[f32], sampling_rate: f32) -> FrequencySpectrum {
    samples_fft_to_spectrum(samples, sampling_rate as u32, FrequencyLimit::All, Some(&divide_by_N)).unwrap()
}

fn update_visualization(new_spectrum: FrequencySpectrum, visualize_spectrum: &RefCell<Vec<(f64, f64)>>) -> Vec<(f64, f64)> {
    // Descriptive constants
    const SMOOTHING_FACTOR: f64 = 0.84;
    const MAGNITUDE_SCALING_FACTOR: f32 = 5000.0;

    new_spectrum.data().iter()
        ...
        .for_each(|((_, fr_val_new), (_, fr_val_old))| {
            ...
            let max_val = max(FrequencyValue::from(scaled_new_val), FrequencyValue::from(old_val as f32));
...
fn draw_visualization(draw: &Draw, spectrum: &Ref<Vec<(f64, f64)>>, amplitude: f32, radius: f32, number_of_points: usize) {
...
fn rainbow_circle(draw: &Draw, spectrum: &Ref<Vec<(f64, f64)>>, amplitude: f32, radius: f32, win: Rect) {
...
fn rainbow_circle2(draw: &Draw, spectrum: &Ref<Vec<(f64, f64)>>, amplitude: f32, win: Rect) {
    ...
    for (index, &(_, y)) in spectrum.iter().enumerate().take(half_num_bins) {
        ...
        // Calculate the outer point of the line
        let outer_point = inner_point + Vec2::new(sin_angle, cos_angle) * scaled_magnitude * modifier;
...
fn rainbow_circle3(draw: &Draw, spectrum: &Ref<Vec<(f64, f64)>>, amplitude: f32, win: Rect, line_weight: f32) {

    const TWO_PI: f32 = 2.0 * PI;
    let num_bins = spectrum.len();
    let half_num_bins = num_bins ; // Use half the spectrum for a full circle
    ...
    for (index, &(_, y)) in spectrum.iter().enumerate().take(half_num_bins) {
        ...
        // Calculate the outer point of the line
        let outer_point = inner_point + Vec2::new(sin_angle, cos_angle) * scaled_magnitude * amplitude;
